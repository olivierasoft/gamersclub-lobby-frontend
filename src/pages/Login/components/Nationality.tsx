...
export function Nationality() {
    ...
    function onDropdownItemClick(language: string) {
        setSelectedCountry(countryOptions.find(option => option.language === language)!)
        setDropdownVisible(!dropdownVisible)
    }
    ...
    return (
        <>
            <div ref={buttonRef}
                 ...
                 className="flex w-40 hover:bg-optionHover border mr-10 border-white border-opacity-50 rounded-sm text-white items-center justify-between cursor-pointer">
                <CountryOption language={selectedCountry.language} icon={selectedCountry.icon}/>
                <AiFillCaretUp className={`mr-3 opacity-40 rotate-${deg}`}/>
            </div>
            <CountryOptionDropdown visible={dropdownVisible} buttonRef={buttonRef} dropdownRef={dropdownRef}>
                {
                    countryOptions
                        .filter(option => option.language !== selectedCountry.language)
                        .map((option, i) => <CountryOption onOptionClickCb={onDropdownItemClick} key={i} hoverEffect={true} language={option.language} icon={option.icon}/>)
                }
